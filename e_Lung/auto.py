...
auto_diff = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss,var_list=[l1w,l2w,l3w,l4w,l5w,l6w])
...
with tf.Session(config=config) as sess:
    ...
    for iter in range(num_epoch):
        ...
        for current_batch_index in range(0,len(training_data),batch_size):
            ...
            auto_results = sess.run([loss,auto_diff],feed_dict={x:current_batch_noise,y:current_batch})
